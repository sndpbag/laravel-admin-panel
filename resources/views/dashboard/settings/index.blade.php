    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 fade-in">
        ...
        @can('settings.branding.update')
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-2xl font-bold text-gray-800 mb-6">General Branding</h3>
                
                <form action="{{ route('settings.branding.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-5">
                        <div class="flex flex-col items-center mb-6">
                            <label class="block text-sm font-semibold text-gray-700 mb-2 text-center w-full">Site Logo</label>
                            @php $logo = \Sndpbag\AdminPanel\Models\SiteSetting::get('site_logo'); @endphp
                            <label for="brandingLogoInput" class="cursor-pointer profile-image-container mb-2">
                                @if($logo)
                                    <img id="logo-preview" src="{{ asset('storage/' . $logo) }}" class="w-32 h-32 rounded-full object-cover border-4 border-gray-200">
                                @else
                                    <div id="no-logo-placeholder" class="w-32 h-32 rounded-full border-4 border-dashed border-gray-300 flex items-center justify-center bg-gray-50">
                                        <span id="no-logo-text" class="text-gray-400 text-xs">No Logo</span>
                                    </div>
                                    <img id="logo-preview" src="" class="w-32 h-32 rounded-full object-cover border-4 border-gray-200 hidden">
                                @endif
                            ...
                            <input type="file" id="brandingLogoInput" name="site_logo" onchange="previewBrandingLogo(this)" class="hidden" accept="image/*">
                            <p class="text-sm bg-white dark:bg-gray-800 text-gray-500 text-center w-full">Click image to change</p>
                            @error('site_logo')
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Site / Product Name</label>
                            <input type="text" name="site_name" value="{{ \Sndpbag\AdminPanel\Models\SiteSetting::get('site_name', 'Dashboard') }}"
                                class="w-full bg-white px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none transition">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Sidebar Welcome Message</label>
                            <input type="text" name="site_welcome" value="{{ \Sndpbag\AdminPanel\Models\SiteSetting::get('site_welcome', 'Welcome to your dashboard') }}"
                                class="w-full bg-white px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none transition">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Dashboard Welcome Message</label>
                            <input type="text" name="dashboard_welcome" value="{{ \Sndpbag\AdminPanel\Models\SiteSetting::get('dashboard_welcome', 'Welcome back!') }}"
                                class="w-full bg-white px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none transition">
                        </div>

                        <button type="submit" class="w-full px-6 py-3 rounded-xl text-white font-semibold hover:shadow-lg transition" style="background: var(--primary);">
                            Update Branding
                        </button>
                    </div>
                </form>
